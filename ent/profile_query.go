...
type ProfileQuery struct {
	...
	// eager-loading edges.
	withPhotos *PhotosMetadataQuery
...
func (pq *ProfileQuery) QueryPhotos() *PhotosMetadataQuery {
	query := &PhotosMetadataQuery{config: pq.config}
...
func (pq *ProfileQuery) WithPhotos(opts ...func(*PhotosMetadataQuery)) *ProfileQuery {
	query := &PhotosMetadataQuery{config: pq.config}
...
func (pq *ProfileQuery) sqlAll(ctx context.Context) ([]*Profile, error) {
	...
	if query := pq.withPhotos; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.Photos = []*PhotosMetadata{}
		}
		query.Where(predicate.PhotosMetadata(func(s *sql.Selector) {
			s.Where(sql.InValues(profile.PhotosColumn, fks...))
		}))
